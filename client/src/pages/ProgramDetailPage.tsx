          ...
          <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1fr_0.85fr] lg:items-end">
            ...
            <div className="border-l border-white/20 pl-6">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-lg bg-white/10 p-4"><p className="text-3xl font-semibold">{program.modules.length}</p><p className="mt-1 text-sm text-white/60">Modul perjalanan</p></div>
                <div className="rounded-lg bg-white/10 p-4"><p className="text-3xl font-semibold">{program.outcomes.length}</p><p className="mt-1 text-sm text-white/60">Output utama</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white py-16">
          <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[0.75fr_1.25fr]">
            <div>
              ...
              <p className="mt-5 leading-7 text-slate-600">Modul disusun berurutan agar siswa memiliki konteks sebelum membuat keputusan dan target yang lebih besar.</p>
            </div>
            <ol className="border-t border-slate-200">
              {program.modules.map((module, index) => (
                <li className="grid grid-cols-[3rem_1fr_auto] items-center gap-3 border-b border-slate-200 py-5" key={module}>
                  ...
                  {index === 0 ? <ClipboardCheck className="size-5 text-emerald-600" /> : <LockKeyhole className="size-4 text-slate-400" />}
                </li>
              ))}
            </ol>
          </div>
